...
const devices = [
  {id:'light_liv',label:'Luz Sala',type:'light',power:60,x:140,y:120,w:36,h:36,on:false},
  {id:'light_bed',label:'Luz Quarto',type:'light',power:60,x:640,y:120,w:36,h:36,on:false},
  {id:'fridge',label:'Frigorífico',type:'appliance',power:120,x:140,y:360,w:60,h:40,on:true},
  {id:'tv',label:'TV',type:'appliance',power:100,x:420,y:360,w:60,h:36,on:false},
  {id:'heater',label:'Aquecedor',type:'heater',power:1500,x:640,y:360,w:64,h:36,on:false}
];
...
function draw(){
  ...
  devices.forEach((d,idx)=>{
    ...
    if(d.on){
      // glowing when on
      const grd = ctx.createRadialGradient(d.x+d.w/2, d.y+d.h/2, 4, d.x+d.w/2, d.y+d.h/2, 60);
    ...
    ctx.strokeStyle = '#0b2430'; ctx.strokeRect(d.x,d.y,d.w,d.h);
    ...
    if(d.on){
      ...
      ctx.beginPath();
      ctx.arc(d.x + d.w/2, d.y + d.h/2, 26 + Math.sin(t)*6, 0, Math.PI*2);
      ctx.stroke();
    }
  });
  ...
  if(pct>0.7) meterFill.style.boxShadow = '0 0 18px rgba(255,107,107,0.3)'; else meterFill.style.boxShadow = '';
...
function snapshotClassify(){
  if(!classifier){ mlStatus.textContent = 'Classificador não ativo'; return; }
  mlStatus.textContent = 'A classificar...';
  classifier.classify((err, results)=>{
    if(err){ console.error(err); mlStatus.textContent = 'Erro no classificador'; return; }
    const top = results[0];
